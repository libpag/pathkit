...
struct SkNx<1,T> {
    ...
    AI T operator[](int PkDEBUGCODE(k)) const {
        return fVal;
    }
...
#if !defined(SKNX_NO_SIMD) && PK_CPU_SSE_LEVEL >= PK_CPU_SSE_LEVEL_SSE2
    #include "include/private/SkNx_sse.h"
#elif !defined(SKNX_NO_SIMD) && defined(PK_ARM_HAS_NEON)
