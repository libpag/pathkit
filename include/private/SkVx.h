...
// Use PKVX_USE_SIMD internally to avoid confusing double negation.
// Do not use 'defined' in a macro expansion.
#if !defined(SKNX_NO_SIMD)
#define PKVX_USE_SIMD 1
#else
#define PKVX_USE_SIMD 0
#endif
...
#if defined(_MSC_VER)
#define PKVX_ALWAYS_INLINE __forceinline
#else
#define PKVX_ALWAYS_INLINE __attribute__((always_inline))
#endif
...
struct alignas(N*sizeof(T)) VecStorage {
   PKVX_ALWAYS_INLINE VecStorage() = default;
   PKVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
...
struct VecStorage<4,T> {
   PKVX_ALWAYS_INLINE VecStorage() = default;
   PKVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
   PKVX_ALWAYS_INLINE VecStorage(T x, T y, T z, T w) : lo(x,y), hi(z, w) {}
   PKVX_ALWAYS_INLINE VecStorage(Vec<2,T> xy, T z, T w) : lo(xy), hi(z,w) {}
   PKVX_ALWAYS_INLINE VecStorage(T x, T y, Vec<2,T> zw) : lo(x,y), hi(zw) {}
   PKVX_ALWAYS_INLINE VecStorage(Vec<2,T> xy, Vec<2,T> zw) : lo(xy), hi(zw) {}

   PKVX_ALWAYS_INLINE Vec<2,T>& xy() { return lo; }
   PKVX_ALWAYS_INLINE Vec<2,T>& zw() { return hi; }
   PKVX_ALWAYS_INLINE T& x() { return lo.lo.val; }
   PKVX_ALWAYS_INLINE T& y() { return lo.hi.val; }
   PKVX_ALWAYS_INLINE T& z() { return hi.lo.val; }
   PKVX_ALWAYS_INLINE T& w() { return hi.hi.val; }

   PKVX_ALWAYS_INLINE Vec<2,T> xy() const { return lo; }
   PKVX_ALWAYS_INLINE Vec<2,T> zw() const { return hi; }
   PKVX_ALWAYS_INLINE T x() const { return lo.lo.val; }
   PKVX_ALWAYS_INLINE T y() const { return lo.hi.val; }
   PKVX_ALWAYS_INLINE T z() const { return hi.lo.val; }
   PKVX_ALWAYS_INLINE T w() const { return hi.hi.val; }

   // Exchange-based swizzles. These should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
   PKVX_ALWAYS_INLINE Vec<4,T> yxwz() const { return shuffle<1,0,3,2>(bit_pun<Vec<4,T>>(*this)); }
   PKVX_ALWAYS_INLINE Vec<4,T> zwxy() const { return shuffle<2,3,0,1>(bit_pun<Vec<4,T>>(*this)); }
...
struct VecStorage<2,T> {
   PKVX_ALWAYS_INLINE VecStorage() = default;
   PKVX_ALWAYS_INLINE VecStorage(T s) : lo(s), hi(s) {}
   PKVX_ALWAYS_INLINE VecStorage(T x, T y) : lo(x), hi(y) {}

   PKVX_ALWAYS_INLINE T& x() { return lo.val; }
   PKVX_ALWAYS_INLINE T& y() { return hi.val; }

   PKVX_ALWAYS_INLINE T x() const { return lo.val; }
   PKVX_ALWAYS_INLINE T y() const { return hi.val; }

   // This exchange-based swizzle should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
   PKVX_ALWAYS_INLINE Vec<2,T> yx() const { return shuffle<1,0>(bit_pun<Vec<2,T>>(*this)); }

   PKVX_ALWAYS_INLINE Vec<4,T> xyxy() const {
...
struct alignas(N*sizeof(T)) Vec : public VecStorage<N,T> {
   ...
   PKVX_ALWAYS_INLINE Vec() = default;
   ...
   PKVX_ALWAYS_INLINE Vec(std::initializer_list<T> xs) {
       T vals[N] = {0};
   ...
   PKVX_ALWAYS_INLINE T  operator[](int i) const { return i<N/2 ? this->lo[i] : this->hi[i-N/2]; }
   PKVX_ALWAYS_INLINE T& operator[](int i)       { return i<N/2 ? this->lo[i] : this->hi[i-N/2]; }

   PKVX_ALWAYS_INLINE static Vec Load(const void* ptr) {
       Vec v;
       memcpy(&v, ptr, sizeof(Vec));
       return v;
   }
   PKVX_ALWAYS_INLINE void store(void* ptr) const {
...
struct Vec<1,T> {
   ...
   PKVX_ALWAYS_INLINE Vec(std::initializer_list<T> xs) : val(xs.size() ? *xs.begin() : 0) {}

   PKVX_ALWAYS_INLINE T  operator[](int) const { return val; }
   PKVX_ALWAYS_INLINE T& operator[](int)       { return val; }
...
#if PKVX_USE_SIMD && (defined(__clang__) || defined(__GNUC__))
...
#if PKVX_USE_SIMD && defined(__AVX2__)
...
#endif
#if PKVX_USE_SIMD && defined(__SSE4_1__)
...
#endif
#if PKVX_USE_SIMD && defined(__ARM_NEON)
   if constexpr (N*sizeof(T) == 16) {
...
#endif
#if PKVX_USE_SIMD && defined(__SSE_4_1__)
   if constexpr (N*sizeof(T) == 16) {
...
#endif
#if PKVX_USE_SIMD && defined(__aarch64__)
...
#endif
#if PKVX_USE_SIMD && defined(__wasm_simd128__)
...
#if PKVX_USE_SIMD && defined(__clang__)
...
#if PKVX_USE_SIMD && defined(__AVX__)
   if constexpr (N == 8) {
...
#if PKVX_USE_SIMD && defined(__F16C__)
   if constexpr (N == 8) {
...
SINT std::enable_if_t<std::is_unsigned_v<T>, Vec<N,T>> saturated_add(const Vec<N,T>& x,
                                                                     const Vec<N,T>& y) {
#if PKVX_USE_SIMD && (defined(__SSE__) || defined(__ARM_NEON))
...
#elif PKVX_USE_SIMD && defined(__SSE__)
