...
void GrAATriangulator::makeEvent(SSEdge* edge,
                                 Vertex* v,
                                 SSEdge* other,
                                 Vertex* dest,
                                 EventList* events,
...
static void get_edge_normal(const Edge* e, SkVector* normal) {
    normal->set(PkDoubleToScalar(e->fLine.fA), PkDoubleToScalar(e->fLine.fB));
}
...
void GrAATriangulator::simplifyBoundary(EdgeList* boundary, const Comparator& c) const {
    ...
    for (Edge* e = boundary->fHead; e != nullptr;) {
        ...
        } else if (prevNormal.dot(normal) < 0.0 && (distPrev * distPrev <= kQuarterPixelSq ||
                                                    distNext * distNext <= kQuarterPixelSq)) {
...
void GrAATriangulator::Event::apply(VertexList* mesh,
    ...
    TESS_LOG("collapsing %g, %g (original edge %g -> %g) to %g (%g, %g) alpha %d\n",
             prev->fID,
             next->fID,
             fEdge->fEdge->fTop->fID,
             fEdge->fEdge->fBottom->fID,
             dest->fID,
             fPoint.fX,
             fPoint.fY,
             fAlpha);
...
bool GrAATriangulator::collapseOverlapRegions(VertexList* mesh,
                                              const Comparator& c,
        ...
        for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
            ...
            if (leftOverlap && rightOverlap) {
                ...
                if (!isOuterBoundary) {
                    ...
                    // Ensure winding values match expected scale for the edge type. During merging
                    // of collinear edges in overlap regions, windings are summed and so could
                    // exceed the expected +/-1 for outer and +/-2 for inner that is used to fill
                    // properly during subsequent polygon generation.
                    e->fWinding =
                            PkScalarCopySign(e->fType == EdgeType::kInner ? 2 : 1, e->fWinding);
                }
            }
            e = prev;
        }
...
void GrAATriangulator::strokeBoundary(EdgeList* boundary,
...
void GrAATriangulator::extractBoundaries(const VertexList& inMesh,
                                         VertexList* innerVertices,
                                         const Comparator& c) const {
...
int GrAATriangulator::polysToAATriangles(Poly* polys, std::vector<float>* data) const {
    ...
    if (0 == count64 || count64 > INT32_MAX) {
        return 0;
    }
    int count = count64;
    int vertexStride = 2 + 1;

    data->reserve(count * vertexStride);
    TESS_LOG("emitting %d verts\n", count);
    this->polysToTriangles(polys, data, SkPathFillType::kWinding);
    ...
    for (Vertex* v = fOuterMesh.fHead; v; v = v->fNext) {
        for (Edge* e = v->fFirstEdgeBelow; e; e = e->fNextEdgeBelow) {
            ...
            this->emitTriangle(v0, v1, v2, 0 /*winding*/, data);
            this->emitTriangle(v0, v2, v3, 0 /*winding*/, data);
        }
    }
    return static_cast<int>(data->size()) / vertexStride;
}
}  // namespace pk
