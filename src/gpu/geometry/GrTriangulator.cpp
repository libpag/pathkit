...
static inline void emit_vertex(Vertex* v, bool emitCoverage, std::vector<float>* data) {
    data->push_back(v->fPoint.fX);
    data->push_back(v->fPoint.fY);

    if (emitCoverage) {
        data->push_back(GrNormalizeByteToFloat(v->fAlpha));
    }
}

static void emit_triangle(
        Vertex* v0, Vertex* v1, Vertex* v2, bool emitCoverage, std::vector<float>* data) {
    TESS_LOG("emit_triangle %g (%g, %g) %d\n", v0->fID, v0->fPoint.fX, v0->fPoint.fY, v0->fAlpha);
    TESS_LOG("              %g (%g, %g) %d\n", v1->fID, v1->fPoint.fX, v1->fPoint.fY, v1->fAlpha);
    TESS_LOG("              %g (%g, %g) %d\n", v2->fID, v2->fPoint.fX, v2->fPoint.fY, v2->fAlpha);
#if TESSELLATOR_WIREFRAME
    emit_vertex(v0, emitCoverage, data);
    emit_vertex(v1, emitCoverage, data);
    emit_vertex(v1, emitCoverage, data);
    emit_vertex(v2, emitCoverage, data);
    emit_vertex(v2, emitCoverage, data);
    emit_vertex(v0, emitCoverage, data);
#else
    emit_vertex(v0, emitCoverage, data);
...
static inline void round(SkPoint* p) {
    p->fX = PkScalarRoundToScalar(p->fX * PkFloatToScalar(4.0f)) * PkFloatToScalar(0.25f);
    p->fY = PkScalarRoundToScalar(p->fY * PkFloatToScalar(4.0f)) * PkFloatToScalar(0.25f);
}
...
static inline SkScalar double_to_clamped_scalar(double d) {
    ...
    static const double kMaxLimit = (double)PK_ScalarMax;
    ...
    return PkDoubleToScalar(std::max(-kMaxLimit, std::min(d, kMaxLimit)));
}
...
void GrTriangulator::emitMonotonePoly(const MonotonePoly* monotonePoly,
    ...
    while (v != vertices.fTail) {
        ...
        Vertex* next = v->fNext;
        if (count == 3) {
            this->emitTriangle(prev, curr, next, monotonePoly->fWinding, data);
            return;
        }
...
void GrTriangulator::emitTriangle(
        Vertex* prev, Vertex* curr, Vertex* next, int winding, std::vector<float>* data) const {
    if (winding > 0) {
    ...
    emit_triangle(prev, curr, next, fEmitCoverage, data);
}
...
void GrTriangulator::emitPoly(const Poly* poly, std::vector<float>* data) const {
    if (poly->fCount < 3) {
        return;
    }
...
void GrTriangulator::generateCubicPoints(const SkPoint& p0,
    ...
    const SkPoint q[] = {{PkScalarAve(p0.fX, p1.fX), PkScalarAve(p0.fY, p1.fY)},
                         {PkScalarAve(p1.fX, p2.fX), PkScalarAve(p1.fY, p2.fY)},
                         {PkScalarAve(p2.fX, p3.fX), PkScalarAve(p2.fY, p3.fY)}};
    const SkPoint r[] = {{PkScalarAve(q[0].fX, q[1].fX), PkScalarAve(q[0].fY, q[1].fY)},
                         {PkScalarAve(q[1].fX, q[2].fX), PkScalarAve(q[1].fY, q[2].fY)}};
    const SkPoint s = {PkScalarAve(r[0].fX, r[1].fX), PkScalarAve(r[0].fY, r[1].fY)};
    pointsLeft >>= 1;
...
bool GrTriangulator::mergeEdgesAbove(Edge* edge,
    ...
    if (coincident(edge->fTop->fPoint, other->fTop->fPoint)) {
        TESS_LOG("merging coincident above edges (%g, %g) -> (%g, %g)\n",
                 edge->fTop->fPoint.fX,
                 edge->fTop->fPoint.fY,
                 edge->fBottom->fPoint.fX,
                 edge->fBottom->fPoint.fY);
...
static SkPoint clamp(SkPoint p, SkPoint min, SkPoint max, const Comparator& c) {
    if (c.fDirection == Comparator::Direction::kHorizontal) {
        ...
        return {min.fX < max.fX ? SkTPin(p.fX, min.fX, max.fX) : SkTPin(p.fX, max.fX, min.fX),
                ...
                if (leftPoly && rightPoly) {
                    if (leftPoly == rightPoly) {
                        if (leftPoly->fTail && leftPoly->fTail->fSide == kLeft_Side) {
                            leftPoly = this->makePoly(
                                    &polys, leftPoly->lastVertex(), leftPoly->fWinding);
                            leftEnclosingEdge->fRightPoly = leftPoly;
                        } else {
                            rightPoly = this->makePoly(
                                    &polys, rightPoly->lastVertex(), rightPoly->fWinding);
...
int GrTriangulator::polysToTriangles(Poly* polys, std::vector<float>* vertex) const {
    int64_t count64 = CountPoints(polys, fPath.getFillType());
    if (0 == count64 || count64 > PK_MaxS32) {
        return 0;
    }
    int count = count64;
    ...
    vertex->reserve(count * vertexStride);
    TESS_LOG("emitting %d verts\n", count);
    polysToTriangles(polys, vertex, fPath.getFillType());
    return static_cast<int>(vertex->size()) / vertexStride;
}
}  // namespace pk
