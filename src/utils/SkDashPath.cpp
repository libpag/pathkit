...
// Offsets PK_ScalarNearlyZero or smaller create empty paths when Iter measures length.
// Large values are scaled by PK_ScalarNearlyZero so significant bits change.
...
static bool clip_line(SkPoint pts[2], const SkRect& bounds, SkScalar intervalLength,
    ...
    if (minXY < leftTop) {
        minXY = leftTop - PkScalarMod(leftTop - minXY, intervalLength);
        if (!swapped) {
            minXY -= priorPhase;  // for rectangles, adjust by prior phase
        }
    }
    if (maxXY > rightBottom) {
        maxXY = rightBottom + PkScalarMod(maxXY - rightBottom, intervalLength);
        if (swapped) {
...
static bool cull_path(const SkPath& srcPath, const SkStrokeRec& rec,
    ...
    if (srcPath.isRect(nullptr)) {
        ...
        PkAssertResult(SkPath::kMove_Verb == iter.next(pts));
...
public:
    bool init(const SkPath& src, SkPath* dst, SkStrokeRec* rec,
        ...
        fTangent.scale(PkScalarInvert(pathLength));
        SkPointPriv::RotateCCW(fTangent, &fNormal);
        fNormal.scale(PkScalarHalf(rec->getWidth()));
        ...
        int n = PkScalarCeilToInt(ptCount) << 2;
        dst->incReserve(n);
...
bool SkDashPath::InternalFilter(SkPath* dst, const SkPath& src, SkStrokeRec* rec,
    ...
    if (cull_path(src, *rec, cullRect, intervalLength, &cullPathStorage)) {
        ...
        if (src.isRect(nullptr) && src.isLastContourClosed() && is_even(initialDashIndex)) {
            ...
            SkScalar endPhase = PkScalarMod(pathLength + initialDashLength, intervalLength);
            int index = 0;
            ...
            if (is_even(index) == (endPhase > 0)) {
                ...
                const SkScalar kTinyOffset = PK_ScalarNearlyZero;
