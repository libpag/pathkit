...
bool SkPathStroker::preJoinTo(const SkPoint& currPt, SkVector* normal,
    ...
    if (!set_normal_unitnormal(fPrevPt, currPt, fResScale, fRadius, normal, unitNormal)) {
        if (SkStrokerPriv::CapFactory(SkPaint::kButt_Cap) == SkStrokerPriv::CapFactory(params.cap)) {
            return false;
        }
    ...
    if (fSegmentCount == 0) {
        ...
        fFirstParams = params;  // Save first params for closing
        ...
        SkScalar invMiterLimit = 0;
        if (params.join == SkPaint::kMiter_Join) {
            if (params.miterLimit > PK_Scalar1) {
                invMiterLimit = PkScalarInvert(params.miterLimit);
            }
        }
        SkStrokerPriv::JoinProc joiner = SkStrokerPriv::JoinFactory(params.join);
        joiner(&fOuter, &fInner, fPrevUnitNormal, fPrevPt, *unitNormal,
               fRadius, invMiterLimit, fPrevIsLine, currIsLine);
    }
    fPrevIsLine = currIsLine;
    fPrevParams = params;  // Save params
    return true;
}

void SkPathStroker::postJoinTo(const SkPoint& currPt, const SkVector& normal,
                               const SkVector& unitNormal, const SkStrokeParams& params) {
    fJoinCompleted = true;
    fPrevPt = currPt;
...
void SkPathStroker::finishContour(bool close, bool currIsLine, const SkStrokeParams& params) {
    if (fSegmentCount > 0) {
        SkPoint pt;

        if (close) {
            // Use first params for closing join
            SkScalar invMiterLimit = 0;
            if (fFirstParams.join == SkPaint::kMiter_Join) {
                if (fFirstParams.miterLimit > PK_Scalar1) {
                    invMiterLimit = PkScalarInvert(fFirstParams.miterLimit);
                }
            }
            SkStrokerPriv::JoinProc joiner = SkStrokerPriv::JoinFactory(fFirstParams.join);
            joiner(&fOuter, &fInner, fPrevUnitNormal, fPrevPt,
                   fFirstUnitNormal, fRadius, invMiterLimit,
                   fPrevIsLine, currIsLine);
            fOuter.close();
            ...
            // cap the end
            fInner.getLastPt(&pt);
            SkStrokerPriv::CapProc capper = SkStrokerPriv::CapFactory(params.cap);
            capper(&fOuter, fPrevPt, fPrevNormal, pt,
                   currIsLine ? &fInner : nullptr);
            fOuter.reversePathTo(fInner);
            // cap the start
            SkStrokerPriv::CapProc firstCapper = SkStrokerPriv::CapFactory(fFirstParams.cap);
            firstCapper(&fOuter, fFirstPt, -fFirstNormal, fFirstOuterPt,
...
SkPathStroker::SkPathStroker(const SkPath& src, SkScalar radius, SkScalar resScale,
...
void SkPathStroker::lineTo(const SkPoint& currPt, const SkStrokeParams& params, 
                           const SkPath::Iter* iter) {
    bool teenyLine = SkPointPriv::EqualsWithinTolerance(fPrevPt, currPt, PK_ScalarNearlyZero * fInvResScale);
    if (SkStrokerPriv::CapFactory(SkPaint::kButt_Cap) == SkStrokerPriv::CapFactory(params.cap) && teenyLine) {
        return;
    }
    ...
    if (!this->preJoinTo(currPt, &normal, &unitNormal, true, params)) {
        return;
    }
    this->line_to(currPt, normal);
    this->postJoinTo(currPt, normal, unitNormal, params);
}
...
void SkPathStroker::conicTo(const SkPoint& pt1, const SkPoint& pt2, SkScalar weight,
    ...
    if (kDegenerate_ReductionType == reductionType) {
        this->lineTo(reduction, params);
        SkStrokeParams roundParams(params.miterLimit, params.cap, SkPaint::kRound_Join);
        this->lineTo(pt2, roundParams);
        return;
    }
    SkVector normalAB, unitAB, normalBC, unitBC;
    if (!this->preJoinTo(pt1, &normalAB, &unitAB, false, params)) {
        this->lineTo(pt2, params);
        return;
    }
    ...
    this->postJoinTo(pt2, normalBC, unitBC, params);
}

void SkPathStroker::quadTo(const SkPoint& pt1, const SkPoint& pt2, const SkStrokeParams& params) {
...
void SkPathStroker::setRayPts(const SkPoint& tPt, SkVector* dxy, SkPoint* onPt,
    ...
    SkScalar axisFlip = PkIntToScalar(fStrokeType);  // go opposite ways for outer, inner
...
SkPathStroker::ResultType SkPathStroker::strokeCloseEnough(const SkPoint stroke[3],
        const SkPoint ray[2], SkQuadConstruct* quadPts, int depth) const {
...
void SkPathStroker::cubicTo(const SkPoint& pt1, const SkPoint& pt2,
                            const SkPoint& pt3, const SkStrokeParams& params) {
    ...
    if (kDegenerate_ReductionType <= reductionType && kDegenerate3_ReductionType >= reductionType) {
        this->lineTo(reduction[0], params);
        SkStrokeParams roundParams(params.miterLimit, params.cap, SkPaint::kRound_Join);
        if (kDegenerate2_ReductionType <= reductionType) {
            this->lineTo(reduction[1], roundParams);
        }
        if (kDegenerate3_ReductionType == reductionType) {
            this->lineTo(reduction[2], roundParams);
        }
        this->lineTo(pt3, roundParams);
        return;
    }
    SkVector normalAB, unitAB, normalCD, unitCD;
    if (!this->preJoinTo(*tangentPt, &normalAB, &unitAB, false, params)) {
        this->lineTo(pt3, params);
        return;
    }
    SkScalar tValues[2];
    ...
    this->postJoinTo(pt3, normalCD, unitCD, params);
}
