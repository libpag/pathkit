...
bool SkStroke::StrokePathWithMultiParams(const SkPath& src,
                                         SkPath* dst,
                                         SkScalar width,
    ...
    if (!dst) {
        return false;  // dst cannot be null
    }
    if (params.empty()) {
        return false;  // params cannot be empty
    }
    if (width < 0) {
        return false;  // width cannot be negative
    }
    if (resScale <= 0) {
        return false;  // resScale must be positive
    }
    ...
    if (radius <= 0) {
        return false;  // effective radius too small
    }
    ...
    auto getCurrentParams = [&params, &segmentIndex]() -> const SkStrokeParams& {
        return params[segmentIndex % params.size()];
    };
