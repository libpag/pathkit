...
struct SkConic {
    ...
    bool PK_WARN_UNUSED_RESULT chopAt(SkScalar t, SkConic dst[2]) const;
     ...
     */
    int PK_SPI computeQuadPOW2(SkScalar tol) const;

    /**
     *  Chop this conic into N quads, stored continguously in pts[], where
     *  N = 1 << pow2. The amount of storage needed is (1 + 2 * N)
     */
    int PK_SPI PK_WARN_UNUSED_RESULT chopIntoQuadsPOW2(SkPoint pts[], int pow2) const;
    ...
    static int BuildUnitArc(const SkVector& start,
                            const SkVector& stop,
                            SkRotationDirection,
                            const SkMatrix*,
