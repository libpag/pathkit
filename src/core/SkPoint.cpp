...
bool SkPoint::normalize() {
    return this->setLength(fX, fY, PK_Scalar1);
}

bool SkPoint::setNormalize(SkScalar x, SkScalar y) {
    return this->setLength(x, y, PK_Scalar1);
}
...
SkScalar SkPoint::Length(SkScalar dx, SkScalar dy) {
    ...
    if (SkScalarIsFinite(mag2)) {
        return pk_float_sqrt(mag2);
    } else {
        double xx = dx;
        double yy = dy;
        return pk_double_to_float(sqrt(xx * xx + yy * yy));
    }
}
