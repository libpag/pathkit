...
bool SkOpSpan::sortableTop(SkOpContour* contourHead) {
    ...
    for (int index = 0; index < count; ++index) {
        ...
        bool ccw = ccw_dxdy(hit->fSlope, dir);
//        PkASSERT(!approximately_zero(hit->fT) || !hit->fValid);
        SkOpSpan* span = hit->fSpan;
        if (!span) {
            return false;
        }
...
#if 0
            PkASSERT((hitSegment->isXor() ? (windSum & 1) == (spanSum & 1) : windSum == spanSum)
        ...
        if (oSpanSum == PK_MinS32) {
            span->setOppSum(oppSum);
        } else {
#if 0
            PkASSERT(hitSegment->oppXor() ? (oppSum & 1) == (oSpanSum & 1) : oppSum == oSpanSum
