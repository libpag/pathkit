...
struct SkOpCurve {
    SkPoint fPts[4];
    SkScalar fWeight;
    PkDEBUGCODE(SkPath::Verb fVerb);

    const SkPoint& operator[](int n) const {
        PkASSERT(n >= 0 && n <= SkPathOpsVerbToPoints(fVerb));
        return fPts[n];
    }
    ...
    void set(const SkDQuad& quad) {
        ...
        PkDEBUGCODE(fWeight = 1);
        PkDEBUGCODE(fVerb = SkPath::kQuad_Verb);
    }
    ...
    void set(const SkDCubic& cubic) {
        ...
        PkDEBUGCODE(fWeight = 1);
        PkDEBUGCODE(fVerb = SkPath::kCubic_Verb);
    }
