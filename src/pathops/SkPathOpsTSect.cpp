...
void SkTCoincident::setPerp(const SkTCurve& c1, double t,
    ...
    SkIntersections i  PkDEBUGCODE((c1.globalState()));
...
SkTSpan* SkTSect::addFollowing(
    ...
    PkDEBUGCODE(result->debugSetGlobalState(this->globalState()));
...
const SkTSect* SkTSpan::debugOpp() const {
    return PkDEBUGRELEASE(fDebugSect->debugOpp(), nullptr);
}
...
void SkTSpan::validate() const {
    ...
    PkASSERT(this != fPrev);
    PkASSERT(this != fNext);
    PkASSERT(fNext == nullptr || fNext != fPrev);
    PkASSERT(fNext == nullptr || this == fNext->fPrev);
    PkASSERT(fPrev == nullptr || this == fPrev->fNext);
    this->validateBounded();
#endif
#if DEBUG_T_SECT
    PkASSERT(fBounds.width() || fBounds.height() || fCollapsed);
    PkASSERT(fBoundsMax == std::max(fBounds.width(), fBounds.height()) || fCollapsed == 0xFF);
    PkASSERT(0 <= fStartT);
    PkASSERT(fEndT <= 1);
    PkASSERT(fStartT <= fEndT);
    PkASSERT(fBounded || fCollapsed == 0xFF);
    if (fHasPerp) {
...
void SkTSpan::validateBounded() const {
    ...
    const SkTSpanBounded* testBounded = fBounded;
    while (testBounded) {
        PkDEBUGCODE(const SkTSpan* overlap = testBounded->fBounded);
        PkASSERT(!overlap->fDeleted);
#if DEBUG_T_SECT
        PkASSERT(((this->debugID() ^ overlap->debugID()) & 1) == 1);
        PkASSERT(overlap->findOppSpan(this));
#endif
        testBounded = testBounded->fNext;
    }
#endif
...
void SkTSpan::validatePerpPt(double t, const SkDPoint& pt) const {
    PkASSERT(fDebugSect->fOppSect->fCurve.ptAtT(t) == pt);
}

SkTSect::SkTSect(const SkTCurve& c
        PkDEBUGPARAMS(SkOpGlobalState* debugGlobalState)
    ...
    , fActiveCount(0)
    , fHung(false)
    PkDEBUGPARAMS(fDebugGlobalState(debugGlobalState))
    ...
    PATH_OPS_DEBUG_T_SECT_PARAMS(fDebugAllocatedCount(0))
{
    ...
    fHead = this->addOne();
    PkDEBUGCODE(fHead->debugSetGlobalState(debugGlobalState));
    fHead->init(c);
}
...
SkTSpan* SkTSect::addOne() {
    ...
    PkDEBUGCODE(result->fDebugSect = this);
#ifdef PK_DEBUG
    ...
    result->fStartT = result->fEndT = result->fBoundsMax = PK_ScalarNaN;
...
bool SkTSect::binarySearchCoin(SkTSect* sect2, double tStart,
    ...
    PkDEBUGCODE(work.fDebugSect = this);
...
bool SkTSect::extractCoincident(
    ...
    double oppStartT PK_INIT_TO_AVOID_WARNING;
    double oppEndT PK_INIT_TO_AVOID_WARNING;
    SkTSpan* prev = first->fPrev;
    PkASSERT(first->fCoinStart.isMatch());
    SkTSpan* oppFirst = first->findOppT(first->fCoinStart.perpT());
    PkOPASSERT(last->fCoinEnd.isMatch());
    bool oppMatched = first->fCoinStart.perpT() < first->fCoinEnd.perpT();
    double coinStart;
    PkDEBUGCODE(double coinEnd);
    SkTSpan* cutFirst;
    if (prev && prev->fEndT == startT
        ...
        PkDEBUGCODE(coinStart = first->fStartT);
        PkDEBUGCODE(oppStartT = oppMatched ? oppFirst->fStartT : oppFirst->fEndT);
    }
    // FIXME: incomplete : if we're not at the end, find end of coin
    SkTSpan* oppLast;
    PkOPASSERT(last->fCoinEnd.isMatch());
    oppLast = last->findOppT(last->fCoinEnd.perpT());
    PkDEBUGCODE(coinEnd = last->fEndT);
#ifdef PK_DEBUG
    ...
    PkOPASSERT(oppStartT < oppEndT);
    PkASSERT(coinStart == first->fStartT);
    PkASSERT(coinEnd == last->fEndT);
    if (!oppFirst) {
        *result = nullptr;
        return true;
    }
    PkOPASSERT(oppStartT == oppFirst->fStartT);
    if (!oppLast) {
        *result = nullptr;
        return true;
    }
    PkOPASSERT(oppEndT == oppLast->fEndT);
...
SkTSpan* SkTSect::findCoincidentRun(
    ...
    do {
        if (work->fCoinStart.isMatch()) {
            ...
            work->validatePerpPt(work->fCoinStart.perpT(), work->fCoinStart.perpPt());
#endif
            PkOPASSERT(work->hasOppT(work->fCoinStart.perpT()));
            if (!work->fCoinEnd.isMatch()) {
                break;
            }
            lastCandidate = work;
    ...
    if (hullResult >= 0) {
        if (hullResult == 2) {  // hulls have one point in common
            if (!span->fBounded || !span->fBounded->fNext) {
                PkASSERT(!span->fBounded || span->fBounded->fBounded == oppSpan);
                if (spanStart) {
...
int SkTSect::linesIntersect(SkTSpan* span,
    ...
    SkIntersections thisRayI  PkDEBUGCODE((span->fDebugGlobalState));
    SkIntersections oppRayI  PkDEBUGCODE((span->fDebugGlobalState));
    ...
    if (thisRayI.used() > 1) {
        ...
        for (int tIndex = 0; tIndex < thisRayI.used(); ++tIndex) {
            for (int lIndex = 0; lIndex < (int) PK_ARRAY_COUNT(thisLine.fPts); ++lIndex) {
    ...
    if (oppRayI.used() > 1) {
        ...
        for (int oIndex = 0; oIndex < oppRayI.used(); ++oIndex) {
            for (int lIndex = 0; lIndex < (int) PK_ARRAY_COUNT(oppLine.fPts); ++lIndex) {
    ...
    do {
        ...
        double closest = DBL_MAX;
        int closeIndex PK_INIT_TO_AVOID_WARNING;
        int oppCloseIndex PK_INIT_TO_AVOID_WARNING;
...
void SkTSect::matchedDirCheck(double t, const SkTSect* sect2,
    ...
    if (*calcMatched) {
        PkASSERT(*oppMatched == this->matchedDirection(t, sect2, t2));
    } else {
...
void SkTSect::mergeCoincidence(SkTSect* sect2) {
    ...
    do {
        ...
        do {
            ...
            PkOPASSERT(test->fStartT != smaller->fEndT);
...
void SkTSect::removeAllBut(const SkTSpan* keep,
    ...
    while (testBounded) {
        ...
        if (bounded != keep && !bounded->fDeleted) {
            PkAssertResult(PkDEBUGCODE(!) span->removeBounded(bounded));
    ...
    PkASSERT(!span->fDeleted);
    PkASSERT(span->findOppSpan(keep));
    PkASSERT(keep->findOppSpan(span));
}
...
void SkTSect::validate() const {
    ...
    PkASSERT(count == fActiveCount);
#endif
#if DEBUG_T_SECT
    PkASSERT(fActiveCount <= fDebugAllocatedCount);
    int deletedCount = 0;
    ...
    PkASSERT(fActiveCount + deletedCount == fDebugAllocatedCount);
#endif
}
...
struct SkClosestRecord {
    ...
    bool matesWith(const SkClosestRecord& mate  PkDEBUGPARAMS(SkIntersections* i)) const {
        PkOPOBJASSERT(i, fC1Span == mate.fC1Span || fC1Span->endT() <= mate.fC1Span->startT()
                || mate.fC1Span->endT() <= fC1Span->startT());
        PkOPOBJASSERT(i, fC2Span == mate.fC2Span || fC2Span->endT() <= mate.fC2Span->startT()
    ...
    void reset() {
        fClosest = FLT_MAX;
        PkDEBUGCODE(fC1Span = nullptr);
        PkDEBUGCODE(fC2Span = nullptr);
        PkDEBUGCODE(fC1Index = fC2Index = -1);
    }
...
struct SkClosestSect {
    ...
    bool find(const SkTSpan* span1, const SkTSpan* span2
            PkDEBUGPARAMS(SkIntersections* i)) {
        ...
        for (int index = 0; index < fUsed; ++index) {
            ...
            if (test->matesWith(*record  PkDEBUGPARAMS(i))) {
...
void SkTSect::BinarySearch(SkTSect* sect1,
    ...
    gDumpTSectNum = 0;
#endif
    PkDEBUGCODE(sect1->fOppSect = sect2);
    PkDEBUGCODE(sect2->fOppSect = sect1);
    intersections->reset();
...
//    PkASSERT(between(0, sect, 2));
    ...
    double start1s PK_INIT_TO_AVOID_WARNING;
    double start1e PK_INIT_TO_AVOID_WARNING;
    do {
        ...
        if (!largest2 || (largest1 && (largest1->fBoundsMax > largest2->fBoundsMax
            ...
            PkDEBUGCODE(half1->debugSetGlobalState(sect1->globalState()));
            if (!half1->split(largest1, &sect1->fHeap)) {
                break;
            }
            if (!sect1->trim(largest1, sect2)) {
                PkOPOBJASSERT(intersections, 0);
                return;
            }
            ...
            PkDEBUGCODE(half2->debugSetGlobalState(sect2->globalState()));
    ...
    if (coincident) {
        ...
        PkASSERT(sect2->fCoincident);  // courtesy check : coincidence only looks at sect 1
        do {
            if (!coincident) {
                return;
            }
...
//    }
    ...
    do {
        ...
        while (result2) {
            closest.find(result1, result2  PkDEBUGPARAMS(intersections));
    ...
    PkOPOBJASSERT(intersections, intersections->used() <= sect1->fCurve.maxIntersections());
}

int SkIntersections::intersect(const SkDQuad& q1, const SkDQuad& q2) {
    SkTQuad quad1(q1);
    SkTQuad quad2(q2);
    SkTSect sect1(quad1  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(quad2  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
...
int SkIntersections::intersect(const SkDConic& c, const SkDQuad& q) {
    SkTConic conic(c);
    SkTQuad quad(q);
    SkTSect sect1(conic  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(quad  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
    SkTSect::BinarySearch(&sect1, &sect2, this);
    return used();
}
...
int SkIntersections::intersect(const SkDConic& c1, const SkDConic& c2) {
    SkTConic conic1(c1);
    SkTConic conic2(c2);
    SkTSect sect1(conic1  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(conic2  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
    SkTSect::BinarySearch(&sect1, &sect2, this);
    return used();
}
...
int SkIntersections::intersect(const SkDCubic& c, const SkDQuad& q) {
    SkTCubic cubic(c);
    SkTQuad quad(q);
    SkTSect sect1(cubic  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(quad  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
    SkTSect::BinarySearch(&sect1, &sect2, this);
    return used();
}
...
int SkIntersections::intersect(const SkDCubic& cu, const SkDConic& co) {
    ...
    SkTConic conic(co);
    SkTSect sect1(cubic  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(conic  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
    SkTSect::BinarySearch(&sect1, &sect2, this);
    return used();
...
int SkIntersections::intersect(const SkDCubic& c1, const SkDCubic& c2) {
    SkTCubic cubic1(c1);
    SkTCubic cubic2(c2);
    SkTSect sect1(cubic1  PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(1));
    SkTSect sect2(cubic2   PkDEBUGPARAMS(globalState())  PATH_OPS_DEBUG_T_SECT_PARAMS(2));
    SkTSect::BinarySearch(&sect1, &sect2, this);
    return used();
}
}  // namespace pk