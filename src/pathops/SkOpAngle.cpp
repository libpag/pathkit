...
bool SkOpAngle::after(SkOpAngle* test) {
    ...
    int ltOrder;
    PkASSERT((lh->fSectorMask & fSectorMask) || (rh->fSectorMask & fSectorMask) || -1 == lrOrder);
...
//    PkASSERT(lrOrder >= 0 || ltOrder >= 0 || trOrder >= 0);
...
// If an order is < 0, the pair is already in an opposite plane. Check the remaining pairs.
    ...
    if (ltOrder == 0 && lrOrder == 0) {
        ...
        PkDEBUGCODE(bool lrOpposite = lh->oppositePlanes(rh));
        bool ltOpposite = lh->oppositePlanes(this);
        PkOPASSERT(lrOpposite != ltOpposite);
...
//    PkASSERT(origin == test.fCurveHalf[0]);
...
int SkOpAngle::lineOnOneSide(const SkOpAngle* test, bool useOriginal) {
    PkASSERT(!fPart.isCurve());
    PkASSERT(test->fPart.isCurve());
...
bool SkOpAngle::endsIntersect(SkOpAngle* rh) {
    ...
    for (int index = 0; index < 2; ++index) {
        ...
        minX = minY = PK_ScalarInfinity;
        maxX = maxY = -PK_ScalarInfinity;
...
//    PkASSERT(SkPath::kLine_Verb == verb || sector >= 0);
    return sector;
}
...
int SkOpAngle::orderable(SkOpAngle* rh) {
    ...
    if (!fPart.isCurve()) {
        if (!rh->fPart.isCurve()) {
            ...
            PkASSERT(x_ry != rx_y); // indicates an undetected coincidence -- worth finding earlier
...
void SkOpAngle::set(SkOpSpanBase* start, SkOpSpanBase* end) {
    ...
    setSpans();
    setSector();
    PkDEBUGCODE(fID = start ? start->globalState()->nextAngleID() : -1);
}
...
void SkOpAngle::setSpans() {
    ...
    const SkPoint* pts = segment->pts();
    PkDEBUGCODE(fPart.fCurve.fVerb = SkPath::kCubic_Verb);  // required for SkDCurve debug check
    PkDEBUGCODE(fPart.fCurve[2].fX = fPart.fCurve[2].fY = fPart.fCurve[3].fX = fPart.fCurve[3].fY
            = PK_ScalarNaN);   //  make the non-line part uninitialized
    PkDEBUGCODE(fPart.fCurve.fVerb = segment->verb());  //  set the curve type for real
...
void SkOpAngle::setSector() {
    ...
    if (!fPart.isCurve()) {  // if it's a line or line-like, note that both sectors are the same
        PkASSERT(fSectorStart >= 0);
        fSectorEnd = fSectorStart;
        fSectorMask = 1 << fSectorStart;
        return;
    }
    PkASSERT(SkPath::kLine_Verb != verb);
