...
public:
    ...
    int debugID() const {
        return PkDEBUGRELEASE(fID, -1);
    }

    int debugIndent() const {
        return PkDEBUGRELEASE(fDebugIndent, 0);
    }


    const SkOpAngle* debugAngle(int id) const {
        return PkDEBUGRELEASE(this->globalState()->debugAngle(id), nullptr);
    }
    ...
    SkOpContour* debugContour(int id) const {
        return PkDEBUGRELEASE(this->globalState()->debugContour(id), nullptr);
    }
    ...
    const SkOpPtT* debugPtT(int id) const {
        return PkDEBUGRELEASE(this->globalState()->debugPtT(id), nullptr);
    }

    const SkOpSegment* debugSegment(int id) const {
        return PkDEBUGRELEASE(this->globalState()->debugSegment(id), nullptr);
    }
    ...
    const SkOpSpanBase* debugSpan(int id) const {
        return PkDEBUGRELEASE(this->globalState()->debugSpan(id), nullptr);
    }
    ...
    void debugValidate() const {
        ...
        do {
            ...
            PkASSERT(segment->prev() == prior);
            prior = segment;
        } while ((segment = segment->next()));
        PkASSERT(prior == fTail);
#endif
    }
    ...
    void init(SkOpGlobalState* globalState, bool operand, bool isXor) {
        ...
        fOperand = operand;
        fXor = isXor;
        PkDEBUGCODE(fID = globalState->nextContourID());
    }
    ...
    void reset() {
        ...
        fCount = 0;
        fDone = false;
        PkDEBUGCODE(fBounds.setLTRB(PK_ScalarMax, PK_ScalarMax, PK_ScalarMin, PK_ScalarMin));
        PkDEBUGCODE(fFirstSorted = -1);
        PkDEBUGCODE(fDebugIndent = 0);
    }
...
//        PkASSERT(!fNext == !!contour);
    ...
    void toPartialBackward(SkPathWriter* path) const {
        ...
        do {
            PkAssertResult(segment->addCurveTo(segment->tail(), segment->head(), path));
    ...
    void toPartialForward(SkPathWriter* path) const {
        const SkOpSegment* segment = &fHead;
        do {
            PkAssertResult(segment->addCurveTo(segment->head(), segment->tail(), path));
        } while ((segment = segment->next()));
    }
...
protected:
    ...
    PkDEBUGCODE(int fID);
    PkDEBUGCODE(mutable int fDebugIndent);
};
...
public:
    ...
    void remove(SkOpContour* contour) {
        if (contour == this) {
            PkASSERT(this->count() == 0);
            return;
        }
        PkASSERT(contour->next() == nullptr);
        SkOpContour* prev = this;
        SkOpContour* next;
