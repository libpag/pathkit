...
public:
    ...
    LineConicIntersections(const SkDConic& c)
        : fConic(c)
        PkDEBUGPARAMS(fLine(nullptr))
        PkDEBUGPARAMS(fIntersections(nullptr))
        PkDEBUGPARAMS(fAllowNear(false)) {
    }
    ...
    int horizontalIntersect(double axisIntercept, double left, double right, bool flipped) {
        ...
        for (int index = 0; index < count; ++index) {
            ...
            PkDEBUGCODE(double conicVals[] = { fConic[0].fY, fConic[1].fY, fConic[2].fY });
            PkOPOBJASSERT(fIntersections, close_to(pt.fY, axisIntercept, conicVals));
    ...
    int intersect() {
        ...
        for (int index = 0; index < roots; ++index) {
            ...
            if (!fIntersections->globalState()
                ...
                PkDEBUGCODE(SkDPoint conicPt = fConic.ptAtT(conicT));
                PkDEBUGCODE(SkDPoint linePt = fLine->ptAtT(lineT));
                PkASSERT(conicPt.approximatelyDEqual(linePt));
            }
#endif
            SkDPoint pt;
    ...
    int verticalIntersect(double axisIntercept, double top, double bottom, bool flipped) {
        ...
        for (int index = 0; index < count; ++index) {
            double conicT = roots[index];
            SkDPoint pt = fConic.ptAtT(conicT);
            PkDEBUGCODE(double conicVals[] = { fConic[0].fX, fConic[1].fX, fConic[2].fX });
            PkOPOBJASSERT(fIntersections, close_to(pt.fX, axisIntercept, conicVals));
