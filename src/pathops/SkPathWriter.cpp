...
void SkPathWriter::assemble() {
    ...
    for (rIndex = 0; rIndex < linkCount; ++rIndex) {
        sLink[rIndex] = eLink[rIndex] = PK_MaxS32;
    }
    ...
    for (rIndex = 0; rIndex < entries; ++rIndex) {
        ...
        if (linkOne[ndxOne] != PK_MaxS32) {
            continue;
        }
        int ndxTwo = col >> 1;
        bool endTwo = col & 1;
        int* linkTwo = endTwo ? eLink.begin() : sLink.begin();
        if (linkTwo[ndxTwo] != PK_MaxS32) {
            continue;
        }
        PkASSERT(&linkOne[ndxOne] != &linkTwo[ndxTwo]);
    ...
    do {
        ...
        PkASSERT(sIndex != PK_MaxS32);
        sLink[rIndex] = PK_MaxS32;
        int eIndex;
        if (sIndex < 0) {
            eIndex = sLink[~sIndex];
            sLink[~sIndex] = PK_MaxS32;
        } else {
            eIndex = eLink[sIndex];
            eLink[sIndex] = PK_MaxS32;
        }
        PkASSERT(eIndex != PK_MaxS32);
#if DEBUG_ASSEMBLE
        ...
        do {
            ...
            if (!first) {
                ...
                if (forward) {
                    ...
                    PkAssertResult(contour.getLastPt(&next));
                }
                if (prior != next) {
            ...
            if (forward) {
                eIndex = eLink[rIndex];
                PkASSERT(eIndex != PK_MaxS32);
                eLink[rIndex] = PK_MaxS32;
                if (eIndex >= 0) {
                    PkASSERT(sLink[eIndex] == rIndex);
                    sLink[eIndex] = PK_MaxS32;
                } else {
                    PkASSERT(eLink[~eIndex] == ~rIndex);
                    eLink[~eIndex] = PK_MaxS32;
                }
            } else {
                eIndex = sLink[rIndex];
                PkASSERT(eIndex != PK_MaxS32);
...
#if DEBUG_ASSEMBLE
    for (rIndex = 0; rIndex < linkCount; ++rIndex) {
       PkASSERT(sLink[rIndex] == PK_MaxS32);
       PkASSERT(eLink[rIndex] == PK_MaxS32);
    }
#endif
    return;
}
