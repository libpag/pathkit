...
static SkPoint force_small_to_zero(const SkPoint& pt) {
    SkPoint ret = pt;
    if (PkScalarAbs(ret.fX) < FLT_EPSILON_ORDERABLE_ERR) {
        ret.fX = 0;
    }
    if (PkScalarAbs(ret.fY) < FLT_EPSILON_ORDERABLE_ERR) {
        ret.fY = 0;
    }
    return ret;
}
...
void SkOpEdgeBuilder::addOperand(const SkPath& path) {
    PkASSERT(fPathVerbs.count() > 0 && fPathVerbs.end()[-1] == SkPath::kDone_Verb);
    fPathVerbs.pop();
    fPath = &path;
...
int SkOpEdgeBuilder::preFetch() {
    ...
    for (auto iter : SkPathPriv::Iterate(*fPath)) {
        auto pathVerb = std::get<0>(iter);
            ...
            case SkPath::kQuad_Verb:
                {
                    ...
                    if (vec1.dot(vec2) < 0) {
                        ...
                        if (!SkScalarsAreFinite(&pair[0].fX, PK_ARRAY_COUNT(pair) * 2)) {
                            return false;
                        }
                        for (unsigned index = 0; index < PK_ARRAY_COUNT(pair); ++index) {
            ...
            case SkPath::kCubic_Verb:
                {
                    ...
                    PkASSERT(breaks <= (int) PK_ARRAY_COUNT(splitT));
                    struct Splitsville {
                        double fT[2];
                        SkPoint fPts[4];
                        SkPoint fReduced[4];
                    ...
                    PkASSERT(PK_ARRAY_COUNT(splits) == PK_ARRAY_COUNT(splitT) + 1);
                    ...
                    for (int index = 0; index <= breaks; ++index) {
                        ...
                        int next = index;
                        int breakLimit = std::min(breaks, (int) PK_ARRAY_COUNT(splits) - 1);
