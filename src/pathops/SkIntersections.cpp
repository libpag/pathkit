...
int SkIntersections::insert(double one, double two, const SkDPoint& pt) {
    ...
    PkASSERT(fUsed <= 1 || fT[0][0] <= fT[0][1]);
    int index;
    for (index = 0; index < fUsed; ++index) {
        ...
        if (more_roughly_equal(oldOne, one) && more_roughly_equal(oldTwo, two)) {
            ...
            PkASSERT(one >= 0 && one <= 1);
            PkASSERT(two >= 0 && two <= 1);
    ...
    if (fUsed >= fMax) {
        PkOPASSERT(0);  // FIXME : this error, if it is to be handled at runtime in release, must
    ...
    fT[1][index] = two;
    ++fUsed;
    PkASSERT(fUsed <= PK_ARRAY_COUNT(fPt));
    return index;
}
...
void SkIntersections::removeOne(int index) {
    ...
    memmove(&fT[0][index], &fT[0][index + 1], sizeof(fT[0][0]) * remaining);
    memmove(&fT[1][index], &fT[1][index + 1], sizeof(fT[1][0]) * remaining);
//    PkASSERT(fIsCoincident[0] == 0);
    int coBit = fIsCoincident[0] & (1 << index);
    fIsCoincident[0] -= ((fIsCoincident[0] >> 1) & ~((1 << index) - 1)) + coBit;
    PkASSERT(!(coBit ^ (fIsCoincident[1] & (1 << index))));
