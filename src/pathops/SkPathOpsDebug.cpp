...
#if DEBUG_SORT
int SkPathOpsDebug::gSortCountDefault = PK_MaxS32;
...
#if defined PK_DEBUG || !FORCE_RELEASE
...
struct SkPathOpsDebug::GlitchLog {
    ...
    SpanGlitch* recordCommon(GlitchType type) {
        ...
        glitch->fStartT = PK_ScalarNaN;
        glitch->fEndT = PK_ScalarNaN;
        glitch->fOppStartT = PK_ScalarNaN;
        glitch->fOppEndT = PK_ScalarNaN;
        glitch->fPt = { PK_ScalarNaN, PK_ScalarNaN };
        glitch->fType = type;
        return glitch;
    }
...
void SkPathOpsDebug::CoinDict::add(const CoinDictEntry& key) {
    ...
    for (int index = 0; index < count; ++index) {
        ...
        if (entry->fIteration == key.fIteration && entry->fLineNumber == key.fLineNumber) {
            PkASSERT(!strcmp(entry->fFunctionName, key.fFunctionName));
...
bool SkPathOpsDebug::ValidWind(int wind) {
    return wind > PK_MinS32 + 0xFFFF && wind < PK_MaxS32 - 0xFFFF;
}
...
#endif //  defined PK_DEBUG || !FORCE_RELEASE
...
void SkOpGlobalState::debugAddLoopCount(SkIntersections* i, const SkIntersectionHelper& wt,
    ...
    for (int index = 0; index < (int) PK_ARRAY_COUNT(fDebugLoopCount); ++index) {
...
void SkDQuad::debugSet(const SkDPoint* pts) {
    ...
    PkDEBUGCODE(fDebugGlobalState = nullptr);
}
...
void SkDRect::debugInit() {
    fLeft = fTop = fRight = fBottom = PK_ScalarNaN;
}
...
void SkOpSegment::debugSetCoinT(int index, SkScalar t) const {
    ...
    PkASSERT(fDebugBaseMin >= t || t >= fDebugBaseMax);
    ...
    PkASSERT(fDebugLastMin >= t || t >= fDebugLastMax);
    PkASSERT((t - fDebugBaseMin > 0) == (fDebugLastMin - fDebugBaseMin > 0));
}
#endif
...
void SkOpSegment::debugShowActiveSpans(SkString* str) const {
    ...
    do {
        ...
        if (span->windSum() == PK_MinS32) {
            str->appendf(" windSum=?");
        } else {
            str->appendf(" windSum=%d", span->windSum());
        }
        if (span->oppValue() && span->oppSum() == PK_MinS32) {
            str->appendf(" oppSum=?");
        } else if (span->oppValue() || span->oppSum() != PK_MinS32) {
...
void SkOpSegment::debugShowNewWinding(const char* fun, const SkOpSpan* span, int winding,
    ...
    SkDebugf(" newOppSum=");
    if (oppWinding == PK_MinS32) {
        SkDebugf("?");
    } else {
        SkDebugf("%d", oppWinding);
    }
    SkDebugf(" oppSum=");
    if (span->oppSum() == PK_MinS32) {
        SkDebugf("?");
    } else {
...
void SkOpAngle::debugValidate() const {
    ...
    do {
        ...
        PkASSERT(lastOppXor == -1 || lastOppXor == (int) useXor);
    ...
    PkASSERT(wind == 0 || !SkPathOpsDebug::gRunFail);
    PkASSERT(opp == 0 || !SkPathOpsDebug::gRunFail);
#endif
}
...
void SkOpAngle::debugValidateNext() const {
    ...
    do {
//        PkASSERT_RELEASE(next->fSegment->debugContains(next));
        angles.push_back(next);
        next = next->next();
        if (next == first) {
            break;
        }
        PkASSERT_RELEASE(!angles.contains(next));
        if (!next) {
            return;
        }
    } while (true);
...
void SkCoincidentSpans::debugStartCheck(const SkOpSpanBase* outer, const SkOpSpanBase* over,
    ...
    PkASSERT(coinPtTEnd()->span() == over || !SkOpGlobalState::DebugRunFail());
    PkASSERT(oppPtTEnd()->span() == outer || !SkOpGlobalState::DebugRunFail());
}
#endif
...
void SkOpCoincidence::debugAddOrOverlap(SkPathOpsDebug::GlitchLog* log,
    ...
    PkOPASSERT(!fTop);   // this is (correctly) reversed in addifMissing()
    ...
    PkASSERT(true || !cs || !cs->deleted());
    PkASSERT(true || !os || !os->deleted());
    PkASSERT(true || !ce || !ce->deleted());
    PkASSERT(true || !oe || !oe->deleted());
...
void SkOpCoincidence::debugAddMissing(SkPathOpsDebug::GlitchLog* log, bool* added) const {
    ...
    *added = false;
    ...
    // addifmissing adds to head freely then add saved head in the end
        ...
        PkASSERT(!ocs->deleted());
        const SkOpSegment* outerCoin = ocs->segment();
        PkASSERT(!outerCoin->done());  // if it's done, should have already been removed from list
        const SkOpPtT* oos = outer->oppPtTStart();
        if (oos->deleted()) {
            return;
        }
        const SkOpSegment* outerOpp = oos->segment();
        PkASSERT(!outerOpp->done());
...
//            SkOpSegment* innerOppWritable = const_cast<SkOpSegment*>(innerOpp);
            if (outerCoin == innerCoin) {
                ...
                const SkOpPtT* oce = outer->coinPtTEnd();
                PkASSERT(!oce->deleted());
                const SkOpPtT* ioe = inner->oppPtTEnd();
                PkASSERT(!ioe->deleted());
...
static void DebugCheckBetween(const SkOpSpanBase* next, const SkOpSpanBase* end,
    ...
    PkASSERT(next != end);
    PkASSERT(!next->contains(end) || log);
    if (next->t() > end->t()) {
        using std::swap;
        swap(next, end);
    }
...
static void DebugCheckOverlap(const SkCoincidentSpans* test, const SkCoincidentSpans* list,
    ...
    PkASSERT(coinSeg == test->coinPtTEnd()->segment());
    const SkOpSegment* oppSeg = test->oppPtTStart()->segment();
    PkASSERT(oppSeg == test->oppPtTEnd()->segment());
    PkASSERT(coinSeg != test->oppPtTStart()->segment());
    PkDEBUGCODE(double tcs = test->coinPtTStart()->fT);
    PkASSERT(between(0, tcs, 1));
    PkDEBUGCODE(double tce = test->coinPtTEnd()->fT);
    PkASSERT(between(0, tce, 1));
    PkASSERT(tcs < tce);
    double tos = test->oppPtTStart()->fT;
    PkASSERT(between(0, tos, 1));
    double toe = test->oppPtTEnd()->fT;
    PkASSERT(between(0, toe, 1));
    PkASSERT(tos != toe);
    if (tos > toe) {
        using std::swap;
        swap(tos, toe);
    ...
    do {
        ...
        PkASSERT(tce < lcs || lce < tcs);
        PkASSERT(toe < los || loe < tos);
...
static void DebugValidate(const SkCoincidentSpans* head, const SkCoincidentSpans* opt,
    ...
    while (coin) {
        PkASSERT(SkOpCoincidence::Ordered(coin->coinPtTStart()->segment(),
                coin->oppPtTStart()->segment()));
        PkASSERT(coin->coinPtTStart()->span()->ptT() == coin->coinPtTStart());
        PkASSERT(coin->coinPtTEnd()->span()->ptT() == coin->coinPtTEnd());
        PkASSERT(coin->oppPtTStart()->span()->ptT() == coin->oppPtTStart());
        PkASSERT(coin->oppPtTEnd()->span()->ptT() == coin->oppPtTEnd());
        coin = coin->next();
    }
...
void SkOpSegment::debugValidate() const {
    ...
    do {
        ...
        PkASSERT(span->segment() == this);
        PkASSERT(!prev || prev->upCast()->next() == span);
        PkASSERT(!prev || prev == span->prev());
        prev = span;
    ...
    PkASSERT(count == fCount);
    PkASSERT(done == fDoneCount);
    PkASSERT(count >= fDoneCount);
    PkASSERT(span->final());
    span->debugValidate();
#endif
}
...
bool SkOpSpanBase::debugCoinEndLoopCheck() const {
    ...
    do {
        ...
        PkASSERT(nextCoin == this || nextCoin->fCoinEnd != nextCoin);
...
//         PkASSERT(coin->containsCoinEnd(this));
        return;
    }
    debugValidate();
//     PkASSERT(this != coin);
...
#ifdef PK_DEBUG   // assert if another undeleted entry points to segment
...
const SkOpSpan* SkOpSpanBase::debugStarter(SkOpSpanBase const** endPtr) const {
    ...
    PkASSERT(this->segment() == end->segment());
...
#if DEBUG_VALIDATE
    const SkOpPtT* ptT = &fPtT;
    PkASSERT(ptT->span() == this);
    do {
//        PkASSERT(SkDPoint::RoughlyEqual(fPtT.fPt, ptT->fPt));
        ptT->debugValidate();
        ptT = ptT->next();
    } while (ptT != &fPtT);
    PkASSERT(this->debugCoinEndLoopCheck());
    if (!this->final()) {
        PkASSERT(this->upCast()->debugCoinLoopCheck());
    }
    if (fFromAngle) {
...
bool SkOpSpan::debugCoinLoopCheck() const {
    ...
    do {
        ...
        PkASSERT(nextCoin == this || nextCoin->fCoincident != nextCoin);
        for (int check = 1; check < loop - 1; ++check) {
...
//         PkASSERT(coin->containsCoincidence(this));
        return;
    }
    debugValidate();
//     PkASSERT(this != coin);
...
int SkIntersections::debugCoincidentUsed() const {
    if (!fIsCoincident[0]) {
        PkASSERT(!fIsCoincident[1]);
        return 0;
    }
    int count = 0;
    PkDEBUGCODE(int count2 = 0;)
...
void SkOpPtT::debugAddOpp(const SkOpPtT* opp, const SkOpPtT* oppPrev) const {
    PkDEBUGCODE(const SkOpPtT* oldNext = this->fNext);
    PkASSERT(this != opp);
//    this->fNext = opp;
    PkASSERT(oppPrev != oldNext);
...
void SkOpPtT::debugValidate() const {
    ...
    PkASSERT(fNext);
    PkASSERT(fNext != this);
    PkASSERT(fNext->fNext);
    PkASSERT(debugLoopLimit(false) == 0);
#endif
}
...
#define SUPPORT_RECT_CONTOUR_DETECTION 0
    ...
    PkASSERT(fillType >= SkPathFillType::kWinding && fillType <= SkPathFillType::kInverseEvenOdd);
...
static int debug_paths_draw_the_same(const SkPath& one, const SkPath& two, SkBitmap& bits) {
    ...
    canvas.drawColor(PK_ColorWHITE);
    SkPaint paint;
    canvas.save();
