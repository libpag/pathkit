...
public:
    ...
    PkDEBUGCODE(SkOpGlobalState* globalState() { return fGlobalState; })

    void init(PkDEBUGCODE(SkOpGlobalState* globalState)) {
        sk_bzero(this, sizeof(*this));
        PkDEBUGCODE(fGlobalState = globalState);
    }
    ...
    void setCoinPtTEnd(const SkOpPtT* ptT) {
        PkOPASSERT(ptT == ptT->span()->ptT());
        PkOPASSERT(!fCoinPtTStart || ptT->fT != fCoinPtTStart->fT);
        PkASSERT(!fCoinPtTStart || fCoinPtTStart->segment() == ptT->segment());
        fCoinPtTEnd = ptT;
        ptT->setCoincident();
    }
    ...
    void setCoinPtTStart(const SkOpPtT* ptT) {
        PkOPASSERT(ptT == ptT->span()->ptT());
        PkOPASSERT(!fCoinPtTEnd || ptT->fT != fCoinPtTEnd->fT);
        PkASSERT(!fCoinPtTEnd || fCoinPtTEnd->segment() == ptT->segment());
        fCoinPtTStart = ptT;
        ptT->setCoincident();
    }
    ...
    void setOppPtTEnd(const SkOpPtT* ptT) {
        PkOPASSERT(ptT == ptT->span()->ptT());
        PkOPASSERT(!fOppPtTStart || ptT->fT != fOppPtTStart->fT);
        PkASSERT(!fOppPtTStart || fOppPtTStart->segment() == ptT->segment());
        fOppPtTEnd = ptT;
        ptT->setCoincident();
    }
    ...
    void setOppPtTStart(const SkOpPtT* ptT) {
        PkOPASSERT(ptT == ptT->span()->ptT());
        PkOPASSERT(!fOppPtTEnd || ptT->fT != fOppPtTEnd->fT);
        PkASSERT(!fOppPtTEnd || fOppPtTEnd->segment() == ptT->segment());
        fOppPtTStart = ptT;
        ptT->setCoincident();
    }
...
private:
    ...
    PkDEBUGCODE(SkOpGlobalState* fGlobalState);
};
...
public:
    ...
    const SkOpAngle* debugAngle(int id) const {
        return PkDEBUGRELEASE(fGlobalState->debugAngle(id), nullptr);
    }
    ...
    SkOpContour* debugContour(int id) const {
        return PkDEBUGRELEASE(fGlobalState->debugContour(id), nullptr);
    }
    ...
    const SkOpPtT* debugPtT(int id) const {
        return PkDEBUGRELEASE(fGlobalState->debugPtT(id), nullptr);
    }

    const SkOpSegment* debugSegment(int id) const {
        return PkDEBUGRELEASE(fGlobalState->debugSegment(id), nullptr);
    }
    ...
    const SkOpSpanBase* debugSpan(int id) const {
        return PkDEBUGRELEASE(fGlobalState->debugSpan(id), nullptr);
    }
...
private:
    ...
    static double TRange(const SkOpPtT* overS, double t, const SkOpSegment* coinPtT
                         PkDEBUGPARAMS(const SkOpPtT* overE));
