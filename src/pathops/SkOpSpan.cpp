...
void SkOpPtT::init(SkOpSpanBase* span, double t, const SkPoint& pt, bool duplicate) {
    ...
    fDeleted = false;
    fCoincident = false;
    PkDEBUGCODE(fID = span->globalState()->nextPtTID());
}
...
void SkOpPtT::setDeleted() {
    PkASSERT(this->span()->debugDeleted() || this->span()->ptT() != this);
    PkOPASSERT(!fDeleted);
    fDeleted = true;
}
...
void SkOpSpanBase::initBase(SkOpSegment* segment, SkOpSpan* prev, double t, const SkPoint& pt) {
    ...
    fAligned = true;
    fChased = false;
    PkDEBUGCODE(fCount = 1);
    PkDEBUGCODE(fID = globalState()->nextSpanID());
    PkDEBUGCODE(fDebugDeleted = false);
}
...
void SkOpSpanBase::merge(SkOpSpan* span) {
    ...
    PkASSERT(this->t() != spanPtT->fT);
    PkASSERT(!zero_or_one(spanPtT->fT));
    span->release(this->ptT());
    if (this->contains(span)) {
        PkOPASSERT(0);  // check to see if this ever happens -- should have been found earlier
    ...
    do {
        ...
        do {
            ...
            if (!zero_or_one(inner->fT)) {
                ...
                PkOPASSERT(inner->fT != test->fT);
                if (!zero_or_one(test->fT)) {
                    testBase->upCast()->release(inner);
                } else {
                    segment->markAllDone();  // mark segment as collapsed
                    PkDEBUGCODE(testBase->debugSetDeleted());
                    test->setDeleted();
                    PkDEBUGCODE(innerBase->debugSetDeleted());
                    inner->setDeleted();
                }
            }
...
bool SkOpSpan::insertCoincidence(const SkOpSegment* segment, bool flipped, bool ordered) {
    ...
    PkASSERT(0); // FIXME? if we get here, the span is missing its opposite segment...
#endif
    return true;
}

void SkOpSpan::release(const SkOpPtT* kept) {
    PkDEBUGCODE(fDebugDeleted = true);
    PkOPASSERT(kept->span() != this);
    PkASSERT(!final());
...
void SkOpSpan::setOppSum(int oppSum) {
    PkASSERT(!final());
    if (fOppSum != PK_MinS32 && fOppSum != oppSum) {
        this->globalState()->setWindingFailed();
        return;
    }
    PkASSERT(!DEBUG_LIMIT_WIND_SUM || SkTAbs(oppSum) <= DEBUG_LIMIT_WIND_SUM);
    fOppSum = oppSum;
}

void SkOpSpan::setWindSum(int windSum) {
    PkASSERT(!final());
    if (fWindSum != PK_MinS32 && fWindSum != windSum) {
        this->globalState()->setWindingFailed();
        return;
    }
    PkASSERT(!DEBUG_LIMIT_WIND_SUM || SkTAbs(windSum) <= DEBUG_LIMIT_WIND_SUM);
    fWindSum = windSum;
}
}  // namespace pk