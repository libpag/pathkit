...
public:
    SkIntersections(PkDEBUGCODE(SkOpGlobalState* globalState = nullptr))
        : fSwap(0)
#ifdef PK_DEBUG
        PkDEBUGPARAMS(fDebugGlobalState(globalState))
        , fDepth(0)
#endif
    {
    ...
    void setMax(int max) {
        PkASSERT(max <= (int) PK_ARRAY_COUNT(fPt));
        fMax = max;
    }
