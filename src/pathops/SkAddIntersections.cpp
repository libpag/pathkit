...
bool AddIntersectTs(SkOpContour* test, SkOpContour* next, SkOpCoincidence* coincidence) {
    ...
    do {
        ...
        do {
            ...
            int pts = 0;
            SkIntersections ts { PkDEBUGCODE(test->globalState()) };
            bool swap = false;
            SkDQuad quad1, quad2;
            ...
            for (int pt = 0; pt < pts; ++pt) {
                PkASSERT(ts[0][pt] >= 0 && ts[0][pt] <= 1);
                PkASSERT(ts[1][pt] >= 0 && ts[1][pt] <= 1);
                ...
                PkASSERT(coincidence->globalState()->debugSkipAssert()
            ...
            PkOPOBJASSERT(coincidence, coinIndex < 0);  // expect coincidence to be paired
        } while (wn.advance());
    } while (wt.advance());
    return true;
}
