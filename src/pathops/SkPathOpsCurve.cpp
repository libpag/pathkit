...
void SkDCurve::setConicBounds(const SkPoint curve[3], SkScalar curveWeight,
    ...
    bounds->setLTRB(PkDoubleToScalar(dRect.fLeft),
                    PkDoubleToScalar(dRect.fTop),
                    PkDoubleToScalar(dRect.fRight),
                    PkDoubleToScalar(dRect.fBottom));
}
...
void SkDCurveSweep::setCurveHullSweep(SkPath::Verb verb) {
    ...
    }
    {
        ...
        PkASSERT(s1x3 * s2x1 < 0 || s1x3 * s3x2 < 0);
        if (s3x2 * s2x1 < 0) {
            PkASSERT(s2x1 * s1x3 > 0);
            fSweep[0] = fSweep[1];
            fOrdered = false;
        }
        fSweep[1] = thirdSweep;
